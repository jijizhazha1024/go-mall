...
(
    ...
    `value`           int          NOT NULL COMMENT '优惠值（根据类型：分/百分比）', -- 满减立减通用,折扣通用（例如：满200减50存50，8折存80）
    `min_amount`      int                   DEFAULT 0 COMMENT '最低消费金额（分）', -- 满减立减通用,折扣通用
    `start_time`      timestamp(3) NOT NULL COMMENT '有效期开始',
    `end_time`        timestamp(3) NOT NULL COMMENT '有效期结束',
