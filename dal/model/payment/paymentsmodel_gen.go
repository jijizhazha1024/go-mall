...
type (
	paymentsModel interface {
		...
		FindOneByIdempotencyKey(ctx context.Context, idempotencyKey string) (*Payments, error)
...
func (m *defaultPaymentsModel) FindOneByIdempotencyKey(ctx context.Context, idempotencyKey string) (*Payments, error) {
	var resp Payments
	query := fmt.Sprintf("select %s from %s where `idempotency_key` = ? limit 1", paymentsRows, m.table)
...
func (m *defaultPaymentsModel) Update(ctx context.Context, newData *Payments) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.PreOrderId, newData.OrderId, newData.OriginalAmount, newData.PaidAmount, newData.PaymentMethod, newData.TransactionId, newData.PayUrl, newData.ExpireTime, newData.Status, newData.IdempotencyKey, newData.PaidAt, newData.PaymentId)
	return err
}
